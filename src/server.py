...
class help:
	...
	output['timestamp'] =  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
        ...
        output ['PUT/setup'] = 'Setup the board Example : curl -X PUT  -H "Content-Type: application/json" -d \'{"board_mode":"board","pins":[{"pin":40, "es": "out", "initial": "low"}]}\' http://0.0.0.0:8080/setup'
